...
namespace ZtWebApi.Common
{
    public class Sys
    {
        ...
        /*产生验证码*/
        public static string getRandomCode(int codeLength, Random rand)
        {
        ...
        //<asp:DropDownList > 选中 
        public static void selected(DropDownList select, string value)
        {
            foreach (ListItem li in select.Items)
            {
                if (li.Value.ToString().Equals(value))
                {
                    li.Selected = true;
                }
            }
        ...
        //<asp:DropDownList > 填充数据 
        public static void selectFill(DropDownList select, DataTable dt, string text, string value)
        {
            select.DataSource = dt.DefaultView;
        ...
        {
            if (!string.IsNullOrEmpty(path) && path.IndexOf("upload") > -1)
            {
                string domain = System.Configuration.ConfigurationManager.AppSettings["FILESERVERADDRESS"];
                return domain + "/" + path.Substring(path.IndexOf("upload")).Replace(@"\", "/");
            }
            return null;
        }
    }
